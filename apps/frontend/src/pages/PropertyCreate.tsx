...
const empty = {
  name: '', address: '', city: '', province: '', postalCode: '', country: 'ES',
  rooms: '1', bathrooms: '', maxGuests: '', pricePerNight: '', purchasePrice: '',
  status: 'active', sesCodigoEstablecimiento: '', nrua: '',
  paperlessCorrespondentId: '', cadastralRef: '',
};
...
export default function PropertyCreate() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setError(e.response?.data?.message || 'Error al crear la propiedad');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="p-4 md:p-6 space-y-6">
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <h1 className="text-xl font-bold flex-1">Nueva propiedad</h1>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <FormField label="Habitaciones" required>
            <input type="number" min="1" value={form.rooms} onChange={f('rooms')} className={inputCls} />
          </FormField>
          ...
          <FormField label="Precio de compra (€)">
            <input type="number" min="0" step="0.01" value={form.purchasePrice} onChange={f('purchasePrice')}
              placeholder="Opcional — para cálculo ROI" className={inputCls} />
          </FormField>
          ...
          <FormField label="ID Correspondent Paperless">
            <input type="number" min="1" step="1" value={form.paperlessCorrespondentId} onChange={f('paperlessCorrespondentId')}
              placeholder="ID numérico del correspondent en Paperless-ngx" className={inputCls} />
          </FormField>
          <FormField label="Referencia catastral">
            <input value={form.cadastralRef} onChange={f('cadastralRef')}
              placeholder="Ej: 1234567AB1234A0001AB" className={inputCls} />
          </FormField>
        </div>
      </div>
